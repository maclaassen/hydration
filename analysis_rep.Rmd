...
Having downloaded from Qualtrics selecting numeric values has messed up height and weight measures so downloaded these in a separate CSV
...
### Consumption and hydration status
```{r}
pal <- drygatebrewer::drygate_palette("TakeMeTotheRiver")
...
figure4B <- ggplot(total_means_ses,
                  aes(x = education_cat,
                      y = amount_ml,
                      fill = drink_type_name)) + 
...
## Fig.3 manuscript
```{r}
right_stack <- (figure_3B / figure_3C)

# top row: figure_3A (left) + right_stack (right)
top_row <- figure_3A + right_stack + 
  plot_layout(ncol = 2, widths = c(1, 1.5))

# final combined: top row above figure_3D (figure_3D spans both columns)
combined <- top_row / figure_3D +
  plot_layout(heights = c(1, 1)) +   # tweak heights as desired
  plot_annotation(tag_levels = "a") + theme(legend.position = "top")

combined
ggsave("figure_ABCD_combined.png", combined, width = 12, height = 10, dpi = 300, device = ragg::agg_png)
```
...
pct_ses <- clean_data_wide %>%
  dplyr::filter(!is.na(education_split) & !is.na(urine_color_split)) %>%
...
prop_long <- clean_data_wide %>%
  dplyr::filter(!is.na(education_split) & !is.na(urine_color_split)) %>%
  dplyr::group_by(education_split, urine_color_split) %>%
  dplyr::summarise(n = n(), .groups = "drop") %>%
  dplyr::group_by(education_split) %>%
  dplyr::mutate(p = n / sum(n)) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(urine_color_split = factor(urine_color_split,
                                    levels = c("inadequate", "adequate"))) 

# select colors
pal <- drygatebrewer::drygate_palette("TakeMeTotheRiver")

figure4A_split <- ggplot(prop_long, aes(x = education_split, y = p, fill = urine_color_split)) +
...
# mean per drink type
figure4B_split <- ggplot(total_means_ses_split,
...
figure4A <- ggplot(prop_long, aes(x = education_cat, y = p, fill = urine_color_split)) +
  geom_col(position = "stack", alpha = 1) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_fill_manual(
    values = c("inadequate" = pal[1], "adequate" = pal[3]),
...
combined_ses <- figure4A + figure4B + plot_layout(ncol = 2, widths = c(0.8, 1.2)) + plot_annotation(tag_levels = "a")
#combined_ses

ggsave("figure_4_combined.png", combined_ses, width = 16, height = 9, dpi = 300, device = ragg::agg_png)
```
...
merged_open_clean <- merged_open

# cleanq1
for (col in grep("^color_drink_coded", names(merged_open_clean), value = TRUE)) {
  merged_open_clean[[col]] <- case_when(
    tolower(merged_open_clean[[col]]) %in% c("ν/α", "n/a", "n/a", "n/a") ~ "N/A",
    merged_open_clean[[col]] == "Ν/Α" ~ "N/A",
    TRUE ~ merged_open_clean[[col]]
  )
}

# cleanq2
for (col in grep("^color_health_coded", names(merged_open_clean), value = TRUE)) {
  merged_open_clean[[col]] <- case_when(
    tolower(merged_open_clean[[col]]) %in% c("ν/α", "n/a", "n/a", "n/a") ~ "N/A",
    merged_open_clean[[col]] == "Ν/Α" ~ "N/A",
    TRUE ~ merged_open_clean[[col]]
...
# clean q3
for (col in grep("^Color_own_coded", names(merged_open_clean), value = TRUE)) {
  merged_open_clean[[col]] <- case_when(
    tolower(merged_open_clean[[col]]) %in% c("ν/α", "n/a", "n/a") ~ "N/A",
    merged_open_clean[[col]] == "Ν/Α" ~ "N/A",
    merged_open_clean[[col]] == "ας" ~ "N/A", 
    TRUE ~ merged_open_clean[[col]]
  )
}
```
...
```{r}
# question 1 urine colour and drinking
participant_q1 <- merged_open_clean %>%
  select(response_id, 
         color_drink_coded_1, color_drink_coded_2, color_drink_coded_3) %>%
  mutate(
    mentions_amount = ifelse(
      (!is.na(color_drink_coded_1) & color_drink_coded_1 == "amount") |
        (!is.na(color_drink_coded_2) & color_drink_coded_2 == "amount") |
        (!is.na(color_drink_coded_3) & color_drink_coded_3 == "amount"), 1, 0),
    
    mentions_hydration = ifelse(
      (!is.na(color_drink_coded_1) & color_drink_coded_1 == "hydration") |
        (!is.na(color_drink_coded_2) & color_drink_coded_2 == "hydration") |
        (!is.na(color_drink_coded_3) & color_drink_coded_3 == "hydration"), 1, 0)
  )

urine_color_association <- participant_q1 %>%
  dplyr::summarise(
    total_participants = n(),
    n_amount_only = sum(mentions_amount == 1 & mentions_hydration == 0, na.rm = TRUE),
    n_hydration_only = sum(mentions_amount == 0 & mentions_hydration == 1, na.rm = TRUE),
    n_both = sum(mentions_amount == 1 & mentions_hydration == 1, na.rm = TRUE),
    n_neither = sum(mentions_amount == 0 & mentions_hydration == 0, na.rm = TRUE)
  ) %>%
  mutate(
    pct_amount_only = round(n_amount_only / total_participants * 100, 1),
    pct_hydration_only = round(n_hydration_only / total_participants * 100, 1),
    pct_both = round(n_both / total_participants * 100, 1),
    pct_neither = round(n_neither / total_participants * 100, 1),
    pct_either_amount_hydration = round((n_amount_only + n_hydration_only + n_both) / total_participants * 100, 1)
  )

urine_color_association

# question 3 urine colour and drinking behaviour 
q3_responses <- merged_open_clean %>%
  dplyr::select(response_id, 
         Color_own_coded_1, Color_own_coded_2, Color_own_coded_3) %>%
  pivot_longer(cols = starts_with("Color_own_coded")) %>% 
  dplyr::filter(!is.na(value) & value != "") %>%
  distinct(response_id, value) %>%
  dplyr::count(value) %>%
  mutate(
    total_participants = n_distinct(merged_open_clean$response_id),
    percentage = round(n / total_participants * 100, 1)
  ) %>%
  arrange(desc(n))

q3_responses

# per hydration status
participant_responses <- merged_open_clean %>%
  select(response_id, urine_color_split,
         color_drink_coded_1, color_drink_coded_2, color_drink_coded_3,
         Color_own_coded_1, Color_own_coded_2, Color_own_coded_3) %>%
  mutate(
    q1_mentions_amount_hydration = ifelse(
      (!is.na(color_drink_coded_1) & color_drink_coded_1 %in% c("amount", "hydration")) |
        (!is.na(color_drink_coded_2) & color_drink_coded_2 %in% c("amount", "hydration")) |
        (!is.na(color_drink_coded_3) & color_drink_coded_3 %in% c("amount", "hydration")),
      "Yes", "No"),
    
    q3_drink_more = ifelse(
      (!is.na(Color_own_coded_1) & Color_own_coded_1 == "drink_more") | 
        (!is.na(Color_own_coded_2) & Color_own_coded_2 == "drink_more") | 
        (!is.na(Color_own_coded_3) & Color_own_coded_3 == "drink_more"), 
      1, 0),
    
    hydration_status = urine_color_split
  )

hydration_knowledge_behavior <- participant_responses %>%
  filter(q1_mentions_amount_hydration == "Yes") %>%
  group_by(hydration_status) %>%
  dplyr::summarise(
    n_total = n(),
    n_drink_more = sum(q3_drink_more == 1, na.rm = TRUE),
    pct_drink_more = round(n_drink_more / n_total * 100, 1)
  )

hydration_knowledge_behavior
```
...
### Checking hot drinks
```{r}
clean_data_wide$total_hot_cent <- scale(clean_data_wide$total_hot,center = TRUE, scale = TRUE)
  
hyp1_1_alone <- glm(urine_color_split ~ total_hot_cent, data = clean_data_wide, family = "binomial")
summary(hyp1_1_alone)
exp(coef(hyp1_1_alone))

hyp1_1_hot <- glm(urine_color_split ~ total_water_cent + total_hot_cent, data = clean_data_wide, family = "binomial")
summary(hyp1_1_hot)
exp(coef(hyp1_1_hot))

hyp2_1_hot <- glm(urine_color_split ~ total_ssb_cent + total_hot_cent, data = clean_data_wide, family = "binomial")
summary(hyp2_1_hot)
exp(coef(hyp2_1_hot))

cor.test(clean_data_wide$total_hot_cent, clean_data_wide$total_water_cent)

cor.test(clean_data_wide$total_hot_cent, clean_data_wide$total_sugar)
```
```{r}
knitr::knit_exit()
```